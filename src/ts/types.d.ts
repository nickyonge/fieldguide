...
declare global {
     ...
     */
    type cssNumberOptionalNumber = number | [number, number?];
    /**
     ...
     * @see https://developer.mozilla.org/en-US/docs/Web/SVG/Guides/Content_type#number-optional-number
     */
    type cssIntegerOptionalInteger = cssNumberOptionalNumber;
     ...
     */
    type cssNumberListOfNumbers = number | number[];

    /** Single-char string representing one of the RGBA channels */
    type cssRGBA = 'R' | 'G' | 'B' | 'A';
     ...
     */
    type cssPreserveAspectRatio = 'none' | `x${'Min' | 'Mid' | 'Max'}Y${'Min' | 'Mid' | 'Max'} ${'meet' | 'slice'}`;
     ...
     */
    type cssCrossorigin = 'anonymous' | 'use-credentials' | '';
     ...
     */
    type cssLength = `${cssLengthUnits}${number}`;
    /**
     ...
     */
    type cssLengthPercentage = cssPercentage | cssLength;
     ...
     */
    type xywhInclusion = boolean | 'all' | 'xyOnly' | 'whOnly' | 'widthHeight' | 'widthHeightOnly' | `${'x' | ''}${'y' | ''}${'w' | ''}${'h' | ''}` | 'width' | 'height' | `none`;
}
